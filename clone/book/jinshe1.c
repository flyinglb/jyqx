...
int do_kan(string arg)
{
        ...
        if(!arg || (sscanf(arg, "%s %d", tmp,times)!=2 )) 
            return notify_fail("指令格式：kan jinshe-book1 <次数>\n");

        if (tmp!="jinshe-book1") return notify_fail("指令格式：kan jinshe-book1 <次数>\n");
        ...
        me->improve_skill("jinshe-jian", times*((int)me->query_skill("literate", 1)/3+1));
