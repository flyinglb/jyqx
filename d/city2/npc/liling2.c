...
void relay_emote(object ob, string verb)
{
	...
	switch(verb) {
        ...
        case "shutup":	
        command ("chat "+ ob->query("name") + ",你这个" + RANK_D->query_rude(ob)+"，我恨死你啦！\n");
	message_vision("\n$N娇喘一口气，说道：风儿会为我报仇的。\n", this_object());
...
//造谣谁死了....
	...
	if( (strsrch(msg, "help liling") >= 0)&&(query("shutup") >= -5)) 
        { 
        command ("chat "+ who + "最好啦，谢谢你啊！\n");
        command("chat* 嘻嘻的笑了几声.\n"); 
        this_object()->add("shutup",-1);
        return;
        }
	if( (strsrch(msg, "shutup liling") >= 0)||(strsrch(msg, "李凌闭嘴") >= 0)||(strsrch(msg, "闭嘴李凌") >= 0))
	{	
        if(this_object()->query("shutup")>=5)
           {
           command ("chat "+ who + ",你这个" + "臭流氓，我恨死你啦！\n");
	   message_vision("\n$N娇喘一口气，说道：风儿会为我报仇的。\n", this_object());
	   message_vision("$N说完，化作一缕清风而去。\n", this_object());
	   destruct(this_object());
           }
