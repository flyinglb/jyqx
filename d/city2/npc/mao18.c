...
int ask_book()
{
        ...
        if((int)me->query("weiwang") <= 60) {
                tell_object(me,"茅十八哼了一声说道：你算什么人物？凭什么要给你？\n");
                return 1;
        }
        if(!me->query_temp("mao18_bian")) {
                tell_object(me,"茅十八哼了一声说道：平白无故的我为什么要给你？\n");
                return 1;
        } else if( query("count") < 1 ) {
                 tell_object(me,"茅十八说道：兄弟你来晚了，要不我可以给你些好东西。\n");
                 return 1;
        } else {
                tell_object(me,"茅十八说道：兄弟我看你顺眼，咱俩投缘。这东西就送你了吧！\n");
                me->delete_temp("mao18_bian");
                add("count",-1);
                new("/clone/book/blade_book3")->move(me);
                CHANNEL_D->do_channel(this_object(),"rumor",
                "听说"+ me->query("name") + "得到刀法—高级篇啦！\n");
                return 1;
        }
}
