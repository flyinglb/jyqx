...
//帅
        if( che == 5 ){
                ...
                if( tCOL - sCOL > 2 || tCOL - sCOL < -2 ||
                tROW - sROW > 2 || tROW - sROW < -2 )
                        return notify_fail("这个棋子一次只能走一格。\n");
                if( cc == 1 )
...
string long() {
        ...
        if( me->name() != ob->query("first") ){
                ...
                if( ob->query("name2") ){
                        ...
                        "┃","     ("+ob->query("first")+") ",HIR"红方"NOR+"所吃棋子：" );
