...
void create()
{
        set("short", "老宅后院");
        set("long", @LONG
这里是向阳老宅的后院。满地是发出朽烂霉气的落叶。
院子的中央有一口井，井台(tai) 上刻着些字。院子南面
有一扇门(men)，但已被坍塌的砖瓦挡住了。
LONG );
        set("exits", ([
            "down" : __DIR__"well",
        ]));
...
int do_move(string arg)
{
        ...
        if( arg=="tai")
        {
                if(me->query("family/master_id") != "dongfang bubai") {
                        write("你试图移动井台，结果怎么也转不动。\n");
                        tell_room(this_object(),me->name() + "试图移动井台，结果怎么也转不动。\n",me);
                        return 1;
                }
		write("你试图移动井台，结果周围的景象突然全变了。\n");
		message("vision",
			me->name() + "刚一移动井台，身形突然不见了。\n",
			environment(me), ({me}) );
