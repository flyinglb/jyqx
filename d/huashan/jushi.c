...
int do_move(string arg)
{
        ...
        if( arg=="tai" )
	{
		...
		{
			return notify_fail(
"岳夫人喝道：这位" + RANK_D->query_respect(me) + "，你不能随便翻人家东西。\n");
			return 1;
		}
		if (!query("had_yunu"))
		{
...
"$N轻轻的拉开抽屉，发现里面有一本书...。\n", this_player());
			...
			paper->move(__FILE__);
message("channel:rumor", HIM"【"+HIR+"谣言"+HIM+"】"+HIR+"某人："+this_player()->query("name")+"拿到玉女剑谱啦。\n"NOR, users());
			set("had_yunu", 1);
			return 1;
		}
		message_vision(
...
"$N轻轻的摘下宝剑，哈哈，真开心...。\n", this_player());
			...
			sword->move(me);
message("channel:rumor", HIM"【"+HIR+"谣言"+HIM+"】"+HIR+"某人："+this_player()->query("name")+"抢了一口龙泉剑啦。\n"NOR, users());
			set("had_sword", 1);
			return 1;
		}
		message_vision(
