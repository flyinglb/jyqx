...
void init()
{
        if (wiz_level(this_player(1))<wiz_level("(angel)"))
        {
                this_player()->move("/d/wizard/guest_room");
                return;
        }
...
int do_post(string arg)
{
        ...
        note["title"] = arg;
        note["author"] = this_player()->name(1)+"("+this_player()->query("id")+")";
        note["time"] = time();
...
int do_discard(string arg)
{
        ...
        &&      wiz_level(this_player(1)) < wiz_level("(hufa)"))
