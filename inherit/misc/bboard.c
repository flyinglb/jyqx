...
string long()
{
	...
	for(i=0; i<sizeof(notes); i++)
		msg += sprintf("%s[%2d]" NOR "  %-40s %12s (%s)\n",
			...
			i+1, notes[i]["title"], notes[i]["author"]+"("+notes[i]["id"]+")", ctime(notes[i]["time"])[0..15] );
	return msg;
}
...
int do_post(string arg)
{
	mapping note;
        ...
        if(this_object()->query("wiz_only") && wiz_level(this_player())< query_wiz_level("(wizard)"))
		return notify_fail("请不要在这里乱涂乱画。\n");

        if((int)this_player()->query("mud_age") < 36000)

		 return notify_fail("你太年轻啦，还是以后再留言把。\n");
...
int do_discard(string arg)
{
	...
	if( notes[num]["author"] != (string) this_player(1)->query("name")
        && wiz_level(this_player()) < query_wiz_level("(arch)"))
