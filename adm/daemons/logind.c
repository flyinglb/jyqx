...
private void logon_main(string arg,object ob)
{
        ...
        UPTIME_CMD->main();
        if( get_wizlock() > 0 ) write(BLINK HIR CHINESE_MUD_NAME HIR"目前正在检修中！\n"NOR);
...
//                if( wizardp(usri) )
        ...
        printf(RED"目前共有"YEL"%s"RED"位巫师、"YEL"%s"RED"位玩家在线，还有"
                YEL"%s"RED"位用户正在尝试登录。\n\n"NOR,
...
void get_id(string arg, object ob)
{
        ...
        if( query_wiz_level(SECURITY_D->get_status(arg)) < wiz_lock_level ) {
                write(HIY"对不起，" + MUD_NAME + "目前限制巫师等级 " + WIZ_LOCK_LEVEL
                        + " 以上的人才能连线。\n"NOR);
                destruct(ob);
                return;
        }
        ...
        if( arg=="guest" ) {
                ...
                write(HIY"您的人物储存挡出了一些问题，请利用 guest 人物通知巫师处理。\n"NOR);
                destruct(ob);
                return;
        }
...
void confirm_relogin(string yn, object ob, object user)
{
        ...
        if( yn[0]!='y' && yn[0]!='Y' ) {
                ...
                tell_object(user, HIR"有人从别处( " + query_ip_number(ob)
                        + " )连线取代你所控制的人物。\n"NOR);
...
{
        if( yn=="" ) {
                write(HIG"使用这个名字将会创造一个新的人物，您确定吗(y/n)？"NOR);
                input_to("confirm_id", ob);
                return;
        }
...
void new_password(string pass, object ob)
{
        write("\n");
        if( strlen(pass)<5 ) {
                write(HIY"密码的长度至少要五个字符，请重设您的密码："NOR);
                input_to("new_password", 1, ob);
                return;
        }
        ob->set("password", crypt(pass,0) );
        write(HIM"请再输入一次您的密码，以确认您没记错："NOR);
...
void confirm_password(string pass, object ob)
{
        ...
        if( crypt(pass, old_pass)!=old_pass ) {
                write(HIY"您两次输入的密码并不一样，请重新设定一次密码："NOR);
                input_to("new_password", 1, ob);
                return;
        }

        write(HIG + @TEXT
一个人物的天赋对于他或她所修习的武艺息息相关。新金庸群侠传II中的人物大多具有
以下四项天赋：
...
void get_choice(string yn, object ob, mapping my)
{
		...
		random_gift(my);
			...
			my["str"], my["int"], my["con"], my["dex"]);
                write(HIY"您接受这一组天赋吗？(y/n)"NOR);
                input_to("get_gift", ob, my);
        }
        if (yn[0] == 'y' || yn[0] == 'Y') {
        printf(HIC"\n现在请您按以下顺序自定天赋。\n您的天赋总点数为80，请输入您的"+HIC"〖臂力〗"NOR+"点数(10-30)："NOR);
...
void get_tf1(string gift, object ob, mapping my)
{
	int tf1, rest;
        if(sscanf(gift,"%d",tf1)!=1 || tf1 > 30 || tf1 <10) {
                write(HIY"点数值必须在"+HIR"10-30"NOR+"之间！\n"NOR);
                printf(HIC"您的天赋总点数为80，请输入您的"+HIC"〖臂力〗"NOR+"点数(10-30)："NOR);
                input_to("get_tf1",ob,my);
        }
	else {
	        my["str"]=tf1;
		rest=80-tf1;
                printf(HIY"\n您的天赋点数尚余%d点，请输入您的"+HIC"〖悟性〗"NOR+"点数(10-30)："NOR, rest);
...
void get_gender(string gender, object ob, object user)
{
        ...
        if( gender[0]=='m' || gender[0]=='M' )
                ...
                write(HIY"对不起，您只能选择男性(m)或女性(f)的角色："NOR);
                input_to("get_gender", ob, user);
                return;
        }
...
// end
        ...
        if( !wizardp(user) && !stringp(user->query("title")) ) {// 修正一个小 BUG ( 武当守门JOB )
                mapping uFamily = user->query("family");
                if( !mapp(uFamily) )
                        user->set("title","普通百姓" );
                else    user->set("title",sprintf("%s第%s代%s", uFamily["family_name"],
...
//changed by long
        ...
        sprintf("%s(%s)由%s连线进入，共清除"+reclaim_objects() + " 个变数。", user->name(), user->query("id"),
             query_ip_name(user)) );
...
int do_counter()
{
        ...
        printf(RED"\n自二零零零年十月四日已有"+YEL+chinese_number(number-1)+RED"个访问者，最高在线"YEL+chinese_number(max_online)+RED"人。\n"NOR);
