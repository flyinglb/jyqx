...
/* global vars */
// 本mud的udp端口号，以及udp socket的socket id
private int my_port, socket_id;

// 这个mapping变量包含了mud的基本信息
...
// 这个变量保存了每个dns里的mud的入口，以及关于协议支持的服务的信息
...
int startup_udp()
{
	...
	if (socket_id) return 0;//如果已经创建了socket则不再创建
	...
	err_no = socket_bind(socket_id, my_port);//将该socket绑定到udp端口
	while( err_no == EEADDRINUSE ) {//如果该端口已经在使用则端口号增加
		my_port++;
...
{
	int sock;
//AUX_PATH在/include/net/dns.h里定义
	if (!ACCESS_CHECK(previous_object())
	&& file_name(previous_object())[0..strlen(AUX_PATH) - 1] != AUX_PATH)
		return;
#ifdef DEBUG
	CHANNEL_D->do_channel(this_object(), "debug", sprintf("DNS: sending to %s %s",host,msg));
#endif
	...
	if(socket_write(sock, msg, host + " " + port)!=EESUCCESS)
		CHANNEL_D->do_channel(this_object(),"debug","DNS: socket_write failed!("+host+" "+port+")");
	socket_close(sock);
}
...
void read_callback(int sock, string msg, string addr)
{
	...
	debug("DNS: Got " + msg);
#ifdef DEBUG
	CHANNEL_D->do_channel(this_object(), "debug", sprintf("DNS: get %s from %s",msg,addr));
#endif
	...
	while (i--)
		if (bits[i] && sscanf(bits[i], "%s:%s", name, arg) == 2)
			args[name] = arg;
	args["HOSTADDRESS"] = addr;//重要！这是对端的IP!
	...
	if( flag > 0 )
	{
		(AUX_PATH + func + ".c" )->incoming_request(args);//调用实际的功能函数
	}
#ifdef DEBUG
if ( args["PORTUDP"] != 0 )
	// add for send intermud functions to a channel by ken@mud.szptt.net.cn at 1997-10-18
	CHANNEL_D->do_channel(this_object(),"debug",
		sprintf("DNS: receive %s host:%s port:%s function: %s",
...
void send_shutdown()
{
	...
	socket_close(socket_id);
	CHANNEL_D->do_channel(this_object(), "sys", "Intermud services has been shut-down...\n");
}

string start_message()
{
	return sprintf( "||MUDNAME:%s||NAME:%s||VERSION:%s||DRIVER:%s||MUDLIB:%s"
		"||HOST:%s||PORT:%d||PORTUDP:%d||TIME:%s||USERS:%d||TCP:%s", 
                CHINESE_MUD_NAME,Mud_name(),MUDLIB_VERSION, __VERSION__, MUDLIB_NAME,
		query_host_name(),mud_port(), my_port, ctime(time()),
...
void init_database()
{
	...
	// add by ken@XAJH
	CHANNEL_D->do_channel(this_object(), "sys", "Dns-Master Init success.\n");
...
void create()
{
	...
	CHANNEL_D->do_channel(this_object(), "sys", "Intermud services has been loaded...\n");
}
