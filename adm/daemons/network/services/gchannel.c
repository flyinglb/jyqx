...
nosave string *welcome_site = ({
//                "61.133.95.11",//JYQXII
                "61.156.17.111",//JYQXII
                });
...
//              sprintf("gchannel message sent to %s.", channel));
...
void incoming_request(mapping info)
{
        ...
        if( member_array(info["HOSTADDRESS"],welcome_site) < 0 )
                return;

#ifdef DEBUG
        CHANNEL_D->do_channel(this_object(), "sys",
                sprintf("gchannel message get from %s.", info["HOSTADDRESS"]+" "+
				info["PORTUDP"]));
#endif
        if(!ACCESS_CHECK(previous_object())) return;
        if (info["NAME"]) {
