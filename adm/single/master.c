...
object connect()
{
	...
	if (err) {
		write("ÏÖÔÚÓÐÈËÕýÔÚÐÞ¸ÄÊ¹ÓÃÕßÁ¬Ïß²¿·ÝµÄ³ÌÊ½£¬Çë´ý»áÔÙÀ´¡£\n");
		write(err);
...
{
	efun::shout("ÏµÍ³ºËÐÄ·¢³öÒ»Éù²Ò½Ð£ºÍÛ¡ªÁ¨¡ªßÖ¡ª\n");
	efun::shout("ÏµÍ³ºËÐÄ¸æËßÄã£ºÒªµ±»úÁË£¬×Ô¼º±£ÖØ°É£¡\n");
        write_file("/log/static/REBOOT",sprintf("ÏµÍ³£¨MASTER£©:%d",time()),1);
...
void log_error(string file, string message)
{
	...
	if( ob )  {
		if( wizardp(ob) )
                        efun::write("±àÒëÊ±´íÎó£º" + message+"\n");
		else
			efun::write("[1;31mÄã¾õµÃÄÄ¶ùÓÐµã²»¶Ô¾¢,¿ÉÊÇÈ´Ëµ²»³öÀ´¡£\n[2;37;0m");
                message("channel:debug",sprintf("[1;37m¡¾µ÷ÊÔ¡¿%s£¨%s£©£º%s\n[2;37;0m",ob->query("name"),ob->query("id"),message),filter(users(),(: wizardp($1) :)));
	}
        else 
                message("channel:debug",sprintf("[1;37m¡¾µ÷ÊÔ¡¿%s£¨%s£©£º%s\n[2;37;0m",name,id,message),filter(users(),(: wizardp($1) :)));
...
void destruct_env_of(object ob)
{
	if (!interactive(ob))
		return;
	tell_object(ob, "ÄãËù´æÔÚµÄ¿Õ¼ä±»»ÙÃðÁË¡£\n");
	ob->move(VOID_OB);
}
...
string standard_trace(mapping error, int caught)
{
    ...
    for (i=0, s = sizeof(error["trace"]); i < s; i++) {
		res = sprintf("%sµ÷ÓÃÀ´×Ô£º%s µÄ %s() µÚ %i ÐÐ£¬Îï¼þ£º %O\n",
			res,
...
{
	object ob = this_player(1);
	string errmsg = standard_trace(error, caught);

	if ( ob ) {
		if( wizardp(ob) )
	        	tell_object(ob, errmsg);
		else 
			tell_object(ob,"[1;31mÄã¾õµÃÄÄ¶ùÓÐµã²»¶Ô¾¢,¿ÉÊÇÈ´Ëµ²»³öÀ´¡£\n[2;37;0m");
                message("channel:debug",sprintf("[1;37m¡¾µ÷ÊÔ¡¿%s£¨%s£©£º%s\n[2;37;0m",ob->query("name"),ob->query("id"),errmsg),filter(users(),(: wizardp($1) :)));
	}
        else
                message("channel:debug",sprintf("[1;37m¡¾µ÷ÊÔ¡¿%s£¨%s£©£º%s\n[2;37;0m",name,id,errmsg),filter(users(),(: wizardp($1) :)));
