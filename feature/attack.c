...
int query_enemy_cnt()
{
    ...
    if ((enemy[i]->query("id")=="tong ren") || (enemy[i]->query("id")=="mu ren")) j++;
    return j;
}
...
void fight_ob(object ob)
{
        ...
        if( !env || env->query("no_fight") || env->is_home() )
                return;
