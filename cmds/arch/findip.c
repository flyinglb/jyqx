...
//        string  *dir,  *ppls,  name,  address;
        string  *dir,  name,  address;

        if( !wizardp(me) ) return 0;
        if( !arg ) return notify_fail("指令格式：findip  <使用者姓名>  |  <IP  地址>\n");
        if( strsrch(arg,".") != -1 ) name = arg;
/*
        count  =  strlen(arg);
        while(count--)
                if( arg[count]=='.' )  {
...
*/
        ...
        if( name != arg )  {
                ...
                who  =  new(LOGIN_OB);
                who->set("id",  name);
                if( !who->restore() ) return notify_fail("没有这位使用者。\n");
                write(who->query("name")+"上次从  "+who->query("last_from")+"  上线。\n");
                if( !who->query("last_from") ) return  notify_fail("");
        ...
        i = 0;
        if(who) destruct(who);
        call_out("search_dir",10,count,dir,i,address,me);
        return  1;
}
...
//        string  *ppls,  name,  str,  adr,  s1,  s2,  s3,  s4;
        ...
        if( i==sizeof(dir) )  {
          tell_object(me,"共有"+to_chinese(count)+"位使用者从("+address+")地址上线。\n");
        } else {
